...
def forward_net(net, data, heatmap, paf, heatmap_mask, paf_mask, is_train=True):
    data = data.as_in_context(ctx)
    heatmap = heatmap.as_in_context(ctx)
    heatmap_mask = heatmap_mask.as_in_context(ctx)
    ...
    for stage_j in range(cpm_stages):
        out[stage_j][0] = nd.elemwise_mul(out[stage_j][0], heatmap_mask)
        out[stage_j][1] = nd.elemwise_mul(out[stage_j][1], paf_mask)
        heatmap = nd.elemwise_mul(heatmap, heatmap_mask)
        paf = nd.elemwise_mul(paf, paf_mask)
...
if __name__ == '__main__':
    ...
    for epoch_idx in range(epoches):
        ...
        for batch_idx, (data, heatmap, paf, heatmap_mask, paf_mask) in enumerate(trainloader):
            losses = forward_net(net, data, heatmap, paf, heatmap_mask, paf_mask, is_train=True)
            ag.backward(losses)
        ...
        for batch_idx, (data, heatmap, paf, heatmap_mask, paf_mask) in enumerate(testloader):
            losses = forward_net(net, data, heatmap, paf, heatmap_mask, paf_mask, is_train=False)
            for rd, loss in zip(rds, losses):
                rd.update(loss.mean().asscalar())
        for rd in rds:
            name, value = rd.get()
